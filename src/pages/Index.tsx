...
const Index = () => {
  ...
  // Effect for mobile infinite scroll
  useEffect(() => {
    if (isMobile && isLoadMoreTriggerVisible && visibleCount < filteredAndSortedMovies.length && !loadingAllMovies) {
      const timer = setTimeout(() => {
        setVisibleCount(prev => prev + BATCH_SIZE);
      }, 300); // Small delay to prevent rapid loading
      return () => clearTimeout(timer);
    }
  }, [isMobile, isLoadMoreTriggerVisible, visibleCount, filteredAndSortedMovies.length, loadingAllMovies]);
