...
const LetterFilterCarousel: React.FC<LetterFilterCarouselProps> = ({ letters, onSelect, activeLetter, className }) => { // Destructure className
  ...
  useEffect(() => {
    ...
    emblaApi.on('resize', onSelectCallback); // Re-evaluate scrollability on resize
  ...
  return (
    <div className={cn("flex items-center flex-grow min-w-0", className)}> {/* Apply className here */}
      {/* Left arrow button and gradient */}
      <div className={cn(
        "relative flex-shrink-0 w-5 h-full", // Reduced width
        canScrollPrev ? "" : "opacity-0 pointer-events-none" // Hide if cannot scroll prev
      )}>
        <div className={cn(
          "absolute inset-0 bg-gradient-to-r from-gray-200 to-transparent pointer-events-none transition-opacity duration-300",
          canScrollPrev ? "opacity-100" : "opacity-0" // Gradient visible only if can scroll prev
        )} />
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className={cn(
        "relative flex-shrink-0 w-5 h-full", // Reduced width
        canScrollNext ? "" : "opacity-0 pointer-events-none" // Hide if cannot scroll next
      )}>
        <div className={cn(
          "absolute inset-0 bg-gradient-to-l from-gray-200 to-transparent pointer-events-none transition-opacity duration-300",
          canScrollNext ? "opacity-100" : "opacity-0" // Gradient visible only if can scroll next
        )} />
        <Button
          variant="ghost"
          size="icon"
