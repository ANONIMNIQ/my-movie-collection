...
interface DynamicMovieCountHeaderProps {
  ...
  allGenres: string[];
  allCountries: string[];
  selectedLetter?: string | null; // New prop for the selected letter
...
const DynamicMovieCountHeader: React.FC<DynamicMovieCountHeaderProps> = ({
  ...
  const displayTitle = React.useMemo(() => {
    if (selectedLetter) {
      return `"${selectedLetter}" Movies`; // Prioritize selected letter
    }
    if (searchQuery) {
      return "Found Movies";
    }
    ...
    return "All Movies";
  }, [selectedLetter, searchQuery, sortAndFilter, allGenres, allCountries]);
