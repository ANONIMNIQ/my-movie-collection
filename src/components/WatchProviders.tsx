...
const WatchProviders: React.FC<WatchProvidersProps> = ({ providers }) => {
  // For simplicity, we'll default to the US region.
  const regionData = providers?.results?.US;

  if (!regionData || (!regionData.flatrate && !regionData.rent && !regionData.buy)) {
    return null; // Don't render anything if no providers are available for the US
  }
      ...
      <Card className="bg-card/50 border-muted-foreground/30">
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="text-sm text-muted-foreground pt-4 border-t border-muted-foreground/20">
            <p>
              Provider information for the US region. Data provided by JustWatch.
              <a
                href={regionData.link}
                target="_blank"
                rel="noopener noreferrer"
