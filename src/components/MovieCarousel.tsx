  ...
  return (
    <section className="mb-12 relative group px-4">
      ...
      <Carousel
        ...
        setApi={setApi}
      >
        <CarouselContent className="pl-20 pr-40 overflow-visible py-12">
          {movies.map((movie) => {
            return (
              <CarouselItem
                key={movie.id}
                className="px-8 basis-1/2 sm:basis-1/3 md:basis-1/4 lg:basis-1/4 xl:basis-1/5 relative overflow-visible"
              >
                <div>
                  <MovieCard
                    movie={movie}
